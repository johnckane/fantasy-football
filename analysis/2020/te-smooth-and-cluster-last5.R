...
te_smoothed_data <- data.frame(pos_adp = c(1:20))

te_smoothed_data$smoothed_cost <- predict(object = loess(data = te_data, 
                                                         adj_value~ pos_adp,
                                                         span = 0.50),
                                          newdata = te_smoothed_data)

te_smoothed_data$smoothed_ppg <- predict(object = loess(data = te_data, ppg ~ pos_adp),
                                         newdata = te_smoothed_data)
te_smoothed_data

## center and scale 
te_smoothed_data_to_scale <- te_smoothed_data[,c(2,3)]
te_smoothed_data_to_scale_scaled <- as.data.frame(scale(te_smoothed_data_to_scale))
colnames(te_smoothed_data_to_scale_scaled) <- c('smoothed_cost_scaled','smoothed_ppg_scaled')
te_smoothed_data_to_scale_scaled  <- bind_cols(te_smoothed_data,te_smoothed_data_to_scale_scaled)
te_smoothed_data_to_scale_scaled


max_clusters <- 10
output <- matrix(0,max_clusters-1,3)
data <- te_smoothed_data_to_scale_scaled
...
te_cluster_data <-
  ...
  group_by(new_cluster) %>%
  mutate(player = paste0("TE-",new_cluster,"-",row_number()))


save(te_cluster_data,
     file = "/home/john/projects/fantasy-football/data/clustering-data/smoothed-and-clustered/te_cluster_data_last5.Rda")
...
save(te_single_obs_per_cluster,
     file = "/home/john/projects/fantasy-football/data/clustering-data/smoothed-and-clustered/te_single_obs_per_cluster_last5.Rda")