...
qb_offense <- inner_join(qbs,offense5,by='player') %>% mutate(name = paste0(fname," ",lname),
                                                              experience = year - start,
                                                              age = year - year(as_date(dob)))
save(qb_offense, file = "/home/john/projects/fantasy-football/data/analysis-data/qb_offense.Rda")
head(qb_offense)
...
qb <-
  ...
  left_join(qb_first8_ranks, by = c('player','name','year','team','experience','age')) %>%
  left_join(qb_last8_ranks, by = c('player','name','year','team','experience','age'))

head(qb)

save(qb, file = "/home/john/projects/fantasy-football/data/analysis-data/qb.Rda")
