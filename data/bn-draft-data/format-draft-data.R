...
draft_data$player <- 
  ifelse(
    draft_data$player == "Le Veon Bell", 
    "Le'Veon Bell",
    draft_data$player
    )
...
draft_data$player <- 
  ifelse(
    draft_data$player == "Steve Smith" & draft_data$team %in% c("BAL","CAR"),
    "Steve Smith Sr.",
    draft_data$player
  )
...
draft_data$player <- 
  ifelse(
    draft_data$pos == "HC",
    str_replace(draft_data$player,"HC",""),
    draft_data$player)
...
uncoded %<>%
  ...
  mutate(player_code = ifelse(player == 'Mike Thomas','MT-0900',
                       ...
                       ifelse(player == 'Josh Allen','JA-1087',
                       ifelse(player == 'Mike Williams' & team == 'LAC','MW-2825',
                       ifelse(player == 'Greg Little', 'GL-0400',
                       ifelse(player == 'DK Metcalf', 'DM-2275',player_code)))))))))


uncoded %>% filter(is.na(player_code) == TRUE) %>% View
...
dbGetQuery(conn = pool,
           statement = "SELECT * 
                        FROM player 
                        WHERE fname = 'Mike' and lname = 'Williams'
                          AND pos1 = 'WR'")
dbGetQuery(conn = pool,
           statement = "SELECT * 
                        FROM player 
                        WHERE fname = 'Greg' and lname = 'Little'")
dbGetQuery(conn = pool,
           statement = "SELECT * 
                        FROM player 
                        WHERE lname = 'Metcalf'")
...
coded_draft_data %<>%
  filter((player == 'Rams D/ST' & year %in% c(2013,2014) & abbr == 'LA') == FALSE,
         (player == 'Rams D/ST' & year %in% c(2015,2016,2018,2019) & abbr == 'STL') == FALSE,
         (player == 'Rams Coach' & year %in% c(2018,2019) & abbr == 'STL') == FALSE)
