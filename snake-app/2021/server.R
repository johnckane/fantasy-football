...
#############################################
  ...
  df <- read.csv("/home/john/projects/fantasy-football/data/FFA/2020/snake_app_data.csv",
...
############################################### 
  ...
  dfr <- reactive({
  if(input$league == "Catalina"){
      df %>% 
      select(1,2,3,4,7,8) %>% 
      data.frame() %>% 
      `colnames<-`(c("player_team","position","age","bye","adp","ppg")) 
    }
  else if(input$league == "Chicago"){
      df %>% 
      select(1,2,3,4,9,10) %>% 
      data.frame() %>% 
      `colnames<-`(c("player_team","position","age","bye","adp","ppg"))
  }
   else if(input$league == "Kane"){
     df %>%
       select(1,2,3,4,5,6) %>%
       data.frame() %>%
       `colnames<-`(c("player_team","position","age","bye","adp","ppg"))
  ...
  observeEvent(input$available_players_rows_selected, {
    str(input$availble_players_rows_selected)
    print(input$available_players_rows_selected)
  }
  )
  ...
  output$available_players <- DT::renderDataTable({
      dfr2() %>%
        filter(!player_team %in% input$drafted_players,
               !player_team %in% input$your_team) %>%
        arrange(adp) %>%
